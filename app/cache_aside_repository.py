...
class CacheAsideRepository:
    ...
    async def insert_heart_rate_data(self, user_id: str, heart_data: list, ttl=3600):
        ...
        keys = [f"heart_rate:{user_id}:latest"]
        await self._invalidate_cache(*keys)
        log.info(f"insert heart rate data - user : {user_id}")
        return records

    async def get_latest_heart_rate_data(self, user_id: str, limit=10, ttl=3600):
        ...
        stmt = select(HeartRateDataDB).where(HeartRateDataDB.user_id == user_id).order_by(desc(HeartRateDataDB.timestamp)).limit(limit)
    ...
    async def get_latest_video_data(self, user_id: str, limit=10, ttl=3600):
        ...
        stmt = select(VideoDataDB).where(VideoDataDB.user_id == user_id).order_by(desc(VideoDataDB.timestamp)).limit(limit)
        result = await self.db_session.execute(stmt)
        records = result.scalars().all()
        data = [
            {
    ...
    async def get_latest_user_data(self, user_id: str, ttl=3600):
        ...
        stmt = select(UserDataDB).where(UserDataDB.user_id == user_id).order_by(desc(UserDataDB.timestamp)).limit(1)
        result = await self.db_session.execute(stmt)
    ...
    async def get_latest_task_data(self, user_id: str, limit=10, ttl=3600):
        ...
        if cache_data:
            ...
            return cache_data
        stmt = select(TaskDataDB).where(TaskDataDB.user_id == user_id).order_by(desc(TaskDataDB.timestamp)).limit(limit)
        result = await self.db_session.execute(stmt)
        records = result.scalars().all()
        data = [
            {
    ...
    async def get_latest_bio_data(self, user_id: str, limit=10, ttl=3600):
        ...
        stmt = select(BioDataDB).where(BioDataDB.user_id == user_id).order_by(desc(BioDataDB.timestamp)).limit(limit)
        result = await self.db_session.execute(stmt)
        records = result.scalars().all()
        data = [
            {
